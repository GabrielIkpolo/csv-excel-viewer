...
export function activate(context: vscode.ExtensionContext) {
  ...
  const openFileCommand = vscode.commands.registerCommand('csvExcelViewer.openFile', (uri: vscode.Uri) => {
    if (uri) {
...
async function parseExcelFile(content: Buffer): Promise<string[][]> {
  // Parse XLSX files (which are ZIP archives containing XML files)
...
// Simple ZIP parser implementation
...
// Simple unzip implementation for deflate compressed data
...
<body>
  ...
  <script>
    ...
    function renderTable() {
      ...
      tableBody.innerHTML = rows.map(row => '<tr>' + row.map(cell => '<td>' + cell + '</td>').join('') + '</tr>').join('');
    }
